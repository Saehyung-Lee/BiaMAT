...
class CIFAR100Data(object):
    ...
    def __init__(self, path):
        ...
        train_images, train_labels = self._load_datafile(os.path.join(path, train_filenames))
        eval_images, eval_labels = self._load_datafile_cv(os.path.join(path, eval_filename))
    ...
    @staticmethod
    def _load_datafile_cv(filename):
      with open(filename, 'rb') as fo:
          if version.major == 3:
              data_dict = pickle.load(fo, encoding='bytes')
          else:
